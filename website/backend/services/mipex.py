...
async def search_mipex(analysis: dict) -> dict:
    ...
    # Ein genannter Ort ohne MIPEX-Wert bekommt NICHTS. Früher kam dann die
    # Top/Bottom-Übersicht — „Integrationspolitik in Nordkorea" bekam Schweden.
    if not primaries and ohne_daten:
        logger.info("mipex: nicht zustaendig fuer %s — kein Ersatzland", ohne_daten)
        return empty
